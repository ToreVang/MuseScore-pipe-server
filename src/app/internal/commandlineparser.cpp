...
void CommandLineParser::init()
{
    ...
    m_parser.addOption(QCommandLineOption("pipe-server",
                                          "Start in pipe-server mode: reads newline-delimited JSON conversion requests from stdin "
                                          "and writes JSON responses to stdout. Keeps the process alive between conversions to "
                                          "eliminate repeated startup overhead. "
                                          "Request format: {\"in\":\"<input>\",\"out\":\"<output>\"[,\"dpi\":<float>][,\"trim\":<int>]}"));
...
void CommandLineParser::parse(int argc, char** argv)
{
    ...
    if (m_parser.isSet("pipe-server")) {
        m_options.runMode = IApplication::RunMode::ConsoleApp;
        m_options.converterTask.type = ConvertType::PipeServer;
    }
